...
public:
	...
	int check_entry_year()
	{
		...
		{
			cin >> year;
			if (1900 <= year && year <= 2038)
			{
				cin_i_cl();
				return year;
			}
			cout << "ОШИБКА - Год поступления должен быть от 1900 до 2038 = ";
			cin_i_cl();
		}
	}
	...
	char check_gender()
	{
		...
		while (true)
		{
			...
			cout << "ОШИБКА - Пол должен быть только Ж для Женщины или М для Mужчины = ";
			cin_c_cl();
		}
	}
...
public:
	...
	void set_date()
	{
		...
		*year = 0;
		...
		{
			if (*(date + i) >= '0' && *(date + i) <= '9' && ((i >= 0 && i <= 1) || (i >= 3 && i <= 4) || (i >= 6 && i <= 9)))
			{
				switch (i)
				{
				case 0: case 1:
	...
	void set_entry_year()
	{
		...
		*entry_year = check_entry_year();
		if (*entry_year < *year + 16)
		{
			cout << "ОШИБКА - Год поступления не может быть меньше или равен году рождения, а также возраст студента в момент поступления должен быть >=16 лет\n";
			set_entry_year();
		}
	}
...
public:
	...
	void get_min_from_file()
	{
		...
		*length = file.tellg();
		...
		cur = new int(0);
		file.seekg(0, ios::beg);
		cout << "\n|_______________Фамилия__________________|__________________Имя___________________|Пол|_Номер_Зачётной_Книжки_|\n";
	...
	bool delete_mrbk_from_file()
	{
		...
		*length = old_file.tellg();
		...
		cout << "\nВведите номер зачётной книжки <10> = ";
		mrbk_numb = check_special_symbols(mrbk_numb, 11);
		cin_c_cl();
		...
		if (rename(path, "delete.bin")) cout << "ОШИБКА - Невозможно переименовать исходный файл.\n";
		if (rename("new_database.bin", path)) cout << "ОШИБКА - Невозможно переименовать новый файл.\n";
		remove("delete.bin");
		return 1;
	}
	...
	void edit_all_use_mrbk_from_file(bool type)
	{
		...
		*length = old_file.tellg();
		...
		while (*cur != *length)
		{
			...
			if (*cur == *begin && type == 0)
			{
				student->set_without_mrbk();
				student->markbook_number = mrbk_numb;
			}
...
public:
	void encrypt()
	{
		...
		pass[64] = 0;
		string command = "openssl\\bin\\openssl.exe enc -aes-256-cbc -salt -in database.bin -out database.enc -pass pass:";
		command += pass;
		system(command.c_str());
		if (remove("database.bin") != 0)
		{
			cout << "ОШИБКА - файл не может быть удален\n";
			return;
		}
		ofstream _file;
		...
		_file.write(pass, 65);
		_file.close();
		string command2 = "openssl\\bin\\openssl.exe rsautl -encrypt -inkey rsa.public -pubin -in passwd.bin -out passwd.enc";
		system(command2.c_str());
		if (remove("passwd.bin") != 0)
		{
			cout << "ОШИБКА - файл не может быть удален\n";
		}
	}
	void decrypt()
	{
		string command = "openssl\\bin\\openssl.exe rsautl -decrypt -inkey rsa.private -in passwd.enc -out passwd.bin";
		system(command.c_str());
		if (remove("passwd.enc") != 0)
		{
			cout << "ОШИБКА - файл не может быть удален\n";
			return;
		}
		...
		_file.read(pass, 65);
		_file.close();
		string command1 = "openssl\\bin\\openssl.exe enc -aes-256-cbc -d -in database.enc -out database.bin -pass pass:";
		command1 += pass;
	...
	void sort()
	{
		...
		*length = file.tellg();
		...
		while (*cur != *length)
		{
			...
			char* _gender = new char('\0');
			*cur += 1;
			file.seekg(138, ios::cur);
			file.read(_gender, 1);
			*cur += 138;

			if ((*_gender == 'Ж' && *gender == 0) || (*_gender == 'М' && *gender == 1))
			{
				...
				*cur += *numb * 26;
				*cur += 1;
				*(*(possition + count) + 1) = (float)*sum_mark / *numb;
				count++;
				delete numb;
				delete tmp;
				delete sum_mark;
		...
		if (*(*(possition + 0) + 0) == 5 && *gender == 0)
		{
			cout << '\n' << "ОШИБКА - Cтудентов женского пола нет.\n";
		}
		else if (*(*(possition + 0) + 0) == 5 && *gender == 1)
		{
			cout << '\n' << "ОШИБКА - Cтудентов мужского пола нет.\n";
		}
		else 
		{
...
public:
	...
	void menu()
	{
		...
		case 6:
			cout << "Выберите вариант\n";
			cout << "1) - Сортировка по полу - только Мужчины\n";
			cout << "2) - Сортировка по полу - только Женщины\n";
			cout << "3) - Сортировка по полу - и Мужчины, и Женщины\n";
			cout << "9) - Назад в меню\n";
			cout << "ВВОД -  ";
			cin >> *answer;
			cin_i_cl();
